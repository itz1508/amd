...
class CategoryValidator:
    ...
    def _validate_mathematical_reasoning(self, prompt: str, answer: str) -> Tuple[bool, list]:
        ...
        # FIX: answer_lower was referenced below but never defined — added here.
        answer_lower = answer.lower()
    ...
    def _validate_sentiment_classification(self, prompt: str, answer: str) -> Tuple[bool, list]:
        ...
        if answer_lower not in allowed_labels and len(answer.split()) == 1:
            errors.append(f"Label must be one of: {allowed_labels}")
            return False, errors

        # FIX: the doc's own category definition is "labelling sentiment AND
        # justifying the classification" — justification is expected by default,
        # not an opt-in the prompt must explicitly request. Only flag extra text
        # as an error if the answer doesn't actually contain one of the valid
        # labels at all (i.e. it's not a label+justification, it's just noise).
        if len(answer.split()) > 1:
            if not any(lbl in answer_lower for lbl in allowed_labels):
                errors.append("Extra text present but no valid sentiment label found in answer")
