...
def is_deterministic_solvable(category: str, prompt: str) -> Tuple[bool, str]:
    ...
    if category == "mathematical_reasoning":
        for pattern in DETERMINISTIC_ARITHMETIC_PATTERNS:
            m = pattern.match(prompt_stripped)
            if m:
                ...
                for ch in expr:
                    if ch == "(":
                        ...
                        op_count += 1
                if op_count <= 1:
                    return True, "Deterministic arithmetic (single operation)"

        for pattern in DETERMINISTIC_PERCENTAGE_PATTERNS:
            if pattern.match(prompt_stripped):
                return True, "Deterministic percentage calculation"
